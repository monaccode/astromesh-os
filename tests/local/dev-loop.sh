...
case "${TARGET}" in
  ...
  *)
    die "unknown target '${TARGET}' (use: build | boot | update | rollback | noshell | confine | sandbox | memory | egress | secureboot | machineconfig | mesh | tpm | otel | otel-metrics | criu | schedext | ebpf-rust | ebpf-control | agent-egress | inspect | clean)"
    ;;
esac
log "done (${TARGET})"
