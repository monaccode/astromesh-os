...
# mesh_smbios <role> <node_id> <mesh_ip> <peer_ip> -> the -smbios value (binary machine-config cred)
mesh_smbios() {
    local role="$1" node_id="$2" mesh_ip="$3" peer_ip="$4"
    smbios_machine_config "$(printf 'profile: mesh-%s\nnode_id: %s\nmesh_ip: %s\npeer_ip: %s\n' "${role}" "${node_id}" "${mesh_ip}" "${peer_ip}")"
}

# boot_mesh_node <role> <node_id> <mesh_ip> <peer_ip> <hostfwd_port> <socket_arg> <swtpm_dir> <mac_suffix>
...
boot_mesh_node() {
    local role="$1" node_id="$2" mesh_ip="$3" peer_ip="$4" port="$5" sock="$6" swtpm_dir="$7" macsfx="$8"
    ...
    qemu-system-x86_64 \
        ...
        $(swtpm_qemu_args) \
        -smbios "$(mesh_smbios "${role}" "${node_id}" "${mesh_ip}" "${peer_ip}")" \
